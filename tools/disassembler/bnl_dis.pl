...
# 19.04.2025 jindroush	seems like there is something like quiz type 8 which is same as type 4 (vroubek)
# 08.07.2025 jindroush	very slight changes in outputting quiz types
...
sub quiz_one_quiz()
{
	...
	sysread( IN, $buf, 5 * 2 );
	&mark_rbuf( 5*2 );
	my( $q_type, $q_cnt, $q_asked, $q_verify, $q_oid ) = unpack( "v*", $buf );
	printf( "\t\tqtype:%04X cnt:%04X questions:%04X verify:%04X quiz_intro_oid: %04X\n", $q_type, $q_cnt, $q_asked, $q_verify, $q_oid );

	my %ONE_QUIZ;
	$ONE_QUIZ{ q_type } = sprintf( "0x%04X", $q_type );
	$ONE_QUIZ{ q_asked } = sprintf( "0x%04X", $q_asked );
	$ONE_QUIZ{ q_verify } = sprintf( "0x%04X", $q_verify );
	...
	foreach $ptr ( @ptrs )
	{
		...
		#explicit handling of quiz type 4 and 8
		...
		elsif( $q_type == 0 )
		{
			...
			sysread( IN, $buf, 6 );
			&mark_rbuf( 6 );
			my( $q0_unk, $q0_oid, $oid_cnt ) = unpack( "v*", $buf );
			printf( "quiz_question0 unk:%04X oid_question:%04X replies_oids:[", $q0_unk, $q0_oid );
			sysread( IN, $buf, 2* $oid_cnt );
			&mark_rbuf( 2 * $oid_cnt );
			my @vals = unpack( "v*", $buf );
			print join( ", ", map( sprintf( "0x%04X", $_ ), @vals ) ), "]\n";
			push @quiz_questions, { q0_unk=>sprintf("0x%04X", $q0_unk ), q0_oid=>sprintf("oid_x%04X", $q0_oid), q0_good_reply_oids=>[ map(sprintf("oid_x%04X",$_), @vals )] };
		}
		elsif( $q_type == 1 )
		{
			#question type 1
			sysread( IN, $buf, 6 );
			&mark_rbuf( 6 );
			my( $q1_question, $q1_correct_reply, $oid_cnt ) = unpack( "v*", $buf );
			printf( "quiz_question1 oid_question:%04X oid_correct_reply:%04X replies_oids:[", $q1_question, $q1_correct_reply );
			sysread( IN, $buf, 2* $oid_cnt );
			&mark_rbuf( 2 * $oid_cnt );
			my @vals = unpack( "v*", $buf );
			print join( ", ", map( sprintf( "0x%04X", $_ ), @vals ) ), "]\n";
			push @quiz_questions, { q1_question=>sprintf("0x%04X", $q1_question ), q1_correct_reply=>sprintf("oid_x%04X", $q1_correct_reply), q1_good_reply_oids=>[ map(sprintf("oid_x%04X",$_), @vals )] };
		}
		else
		{
			#unhandled question type
			die "unknown quiz question type, expected 0, 1, 4, 8, got $q_type";
		}
	}
